...
func (module *Module) getFieldSafe(ctx pgsgo.Context, f pgs.Field, vn *varNamer, ix *importTracker, goPrefix string) (*fieldContext, error) {
	...
	case pgs.MessageT:
		...
		case pgdb_v1.FieldOptions_MESSAGE_BEHAVIOR_VECTOR:
			return nil, nil
		case pgdb_v1.FieldOptions_MESSAGE_BEHAVIOR_MIN_HASH:
			// handled in getCommonFields
			return nil, nil
		default:
...
func getCommonFields(ctx pgsgo.Context, m pgs.Message, ix *importTracker) ([]*fieldContext, error) {
	...
	// iterate message for minhash behavior options
	for _, field := range m.Fields() {
		ext := pgdb_v1.FieldOptions{}
		_, err := field.Extension(pgdb_v1.E_Options, &ext)
		if err != nil {
		...
		if ext.GetMessageBehavior() != pgdb_v1.FieldOptions_MESSAGE_BEHAVIOR_MIN_HASH {
			continue
		}
		...
		byteArrayField := getFieldMinHashShape(field)
		...
		tempCtx := &fieldContext{
			...
			DB: &pgdb_v1.Column{
				...
				Type:     "bit",
				Nullable: true,
				OverrideExpression: fmt.Sprintf("bit(%d)", ext.GetMinhashSize()),
			},
			GoName:   ctx.Name(field).String(), // Generated go struct name
			DataType: nil,
			// new struct to implement this
			Convert: &pbMinHashConvert{
				VarName:       vn.String(),
				GoName:        ctx.Name(field).String(),          // Generated go struct name
				ByteArrayName: ctx.Name(byteArrayField).String(), // Generated byte array name
				ByteArraySize: ext.GetMinhashSize(),
			},
			QueryTypeName: ctx.Name(m).String() + ctx.Name(field).String() + "QueryType",
		}
		rv = append(rv, tempCtx)
	}
