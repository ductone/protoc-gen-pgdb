...
func col2spec(col *Column) string {
	...
	if col.OverrideExpression != "" {
		...
		if col.Sequence != nil {
			_, _ = sbuf.WriteString(sequenceToSQL(col.Sequence))
		} else {
			if !col.Nullable {
...
// sequenceToSQL generates the GENERATED AS IDENTITY clause for a sequence column.
func sequenceToSQL(seq *SequenceOptions) string {
	sbuf := &bytes.Buffer{}
	if seq.Always {
		_, _ = sbuf.WriteString(" GENERATED ALWAYS AS IDENTITY")
	} else {
		_, _ = sbuf.WriteString(" GENERATED BY DEFAULT AS IDENTITY")
	}

	// Build sequence options if any are non-default
	seqOpts := &bytes.Buffer{}
	if seq.Start != 0 && seq.Start != 1 {
		_, _ = seqOpts.WriteString(" START WITH ")
		_, _ = seqOpts.WriteString(strconv.FormatInt(seq.Start, 10))
	}
	if seq.Increment != 0 && seq.Increment != 1 {
		_, _ = seqOpts.WriteString(" INCREMENT BY ")
		_, _ = seqOpts.WriteString(strconv.FormatInt(seq.Increment, 10))
	}
	if seq.MinValue != nil {
		_, _ = seqOpts.WriteString(" MINVALUE ")
		_, _ = seqOpts.WriteString(strconv.FormatInt(*seq.MinValue, 10))
	}
	if seq.MaxValue != nil {
		_, _ = seqOpts.WriteString(" MAXVALUE ")
		_, _ = seqOpts.WriteString(strconv.FormatInt(*seq.MaxValue, 10))
	}
	if seq.Cycle {
		_, _ = seqOpts.WriteString(" CYCLE")
	}
	if seq.Cache != nil {
		_, _ = seqOpts.WriteString(" CACHE ")
		_, _ = seqOpts.WriteString(strconv.FormatInt(int64(*seq.Cache), 10))
	}

	if seqOpts.Len() > 0 {
		_, _ = sbuf.WriteString(" (")
		_, _ = sbuf.WriteString(strings.TrimPrefix(seqOpts.String(), " "))
		_, _ = sbuf.WriteString(")")
	}
