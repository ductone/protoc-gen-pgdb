...
func (x *MessageOptions) SetDropEnabled(v bool) {
...
type MessageOptions_builder struct {
	...
	Disabled bool
	// if this message is used then, instead of generating the normal
	// table/query code, we only generate helpers that return the `DROP TABLE`
	// and `TRUNCATE TABLE` statements for this record's table. This is a way to
	...
	//
	// NOT supported on partitioned messages (partitioned / partitioned_by_created_at
	// / partitioned_by_date_range / partitioned_by_ksuid_field_name): DROP INDEX
	// cannot be CONCURRENT on a partitioned parent, so it would take ACCESS
	// EXCLUSIVE on the parent and every partition. Code generation fails if this is
	// set on a partitioned message; drop such an index out-of-band instead.
