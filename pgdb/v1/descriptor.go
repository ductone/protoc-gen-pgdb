...
type Column struct {
	...
	// Sequence contains sequence generator options for auto-incrementing columns.
	// When set, the column uses GENERATED AS IDENTITY in PostgreSQL.
	Sequence *SequenceOptions
}

// SequenceOptions defines sequence generator configuration for identity columns.
type SequenceOptions struct {
	// Always determines the IDENTITY generation mode:
	// - true: GENERATED ALWAYS AS IDENTITY (cannot be overridden)
	// - false: GENERATED BY DEFAULT AS IDENTITY (can be overridden)
	Always bool
	// Start is the starting value for the sequence (default: 1).
	Start int64
	// Increment is the step value for the sequence (default: 1).
	Increment int64
	// MinValue is the minimum value for the sequence (optional).
	MinValue *int64
	// MaxValue is the maximum value for the sequence (optional).
	MaxValue *int64
	// Cycle determines if the sequence should restart when reaching limits.
	Cycle bool
	// Cache is the number of values to pre-allocate for performance (optional).
	Cache *int32
}
